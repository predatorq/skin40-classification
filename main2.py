...
class Skin40(Dataset):
    ...
    def __init__(self, root, mode, n_augment=2):
...
if __name__ == '__main__':
    ...
    training_set = Skin40(train_dir, 'train', 3)
    test_set = Skin40(test_dir, 'test', 3)

    # Define data loaders for training and testing data in this fold
    trainloader = torch.utils.data.DataLoader(
        training_set,
        batch_size=8, shuffle=True, num_workers=os.cpu_count())
    ...
    network = ViT('B_16_imagenet1k', pretrained=True)
    ...
    optimizer = torch.optim.AdamW(
        network.parameters(), lr=1e-4, weight_decay=1e-3)
    schedulr = torch.optim.lr_scheduler.StepLR(
         optimizer, step_size=1, gamma=0.88)
